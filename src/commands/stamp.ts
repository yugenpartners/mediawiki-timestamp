...
export default class Stamp extends Command {
  ...
  async run() {
    ...
    this.log(`Loaded ${data.length} bytes exported from ${siteinfo.siteinfo.sitename} running ${siteinfo.siteinfo.generator}`);
  ...
  async parseElement(doc: libxmljs.Document, xpath: string): Promise<object> {
    return await xml2js.parseStringPromise(<string>this.getElement(doc, xpath).toString());
  }
}
